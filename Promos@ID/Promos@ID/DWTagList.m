...
#define BACKGROUND_COLOR [UIColor colorWithRed:0 green:0 blue:0 alpha:0.5]
...
- (void)setTags:(NSArray *)array:(NSArray *)userarray
{
    ...
    if (automaticResize) {
        ...
        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, screenWidth*0.72, sizeFit.height);
    }
    else {
        [self display];
    }
...
- (void)touchDownInside:(id)sender
{
    ...
    if([[button superview].backgroundColor isEqual:[UIColor orangeColor]]){
        [[button superview] setBackgroundColor:[self getBackgroundColor]];
    }
    else [[button superview] setBackgroundColor:[UIColor orangeColor]];
}
