...
- (id)initWithFrame:(CGRect)frame {
  if (self = [super initWithFrame:frame]) {
    ...
    NSString *catFilename = [NSString stringWithFormat:@"cat%d.jpg", pickACat];
    //UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:catFilename]];
      _displayThumb=[[UIImageView alloc] initWithImage:[UIImage imageNamed:catFilename]];
    // Scale with fill for contents when we resize.
    _displayThumb.contentMode = UIViewContentModeScaleAspectFill;
    ...
    _displayThumb.frame = imageViewFrame;
    _displayThumb.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    _displayThumb.clipsToBounds = YES;
    [self.contentView addSubview:_displayThumb];
  }
  return self;
}
